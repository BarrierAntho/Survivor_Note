...
const checkDogs = function(dogsJulia, dogsKate) {
		...
		dogs.forEach(function(dog, i) {
				dog < 3 ? console.log(`Dog number ${i + 1} is still a puppy`) : console.log(`Dog number ${i + 1} is an adult`);
		});
};
...
const calcAverageHumanAge = function(ages) {
  ...
  console.log(adults);
  const averages = adults.reduce((acc, current) => acc + current, 0) / adults.length;
...
const checkedEatingDay = dog => dog.curFood > (dog.recommendedFood * 0.90) && dog.curFood < (dog.recommendedFood * 1.10);
console.log(dogs.some(checkedEatingDay));

// 7.
console.log(dogs.filter(checkedEatingDay));
